...
def bill_to_date_from_session():
    ...
    # ----- testing/feedback -----
    nan_dates_size = bill_to_date_from_session[bill_to_date_from_session['date'].isnull()].size
    if nan_dates_size == 0:
        feedback = 'There are no relevant sessions w/o date data.'
    else:
        errors = bill_to_date_from_session[bill_to_date_from_session['date'].isnull()]
        errors = str(errors.to_dict())
        feedback = 'Relevant sessions w/o date data:' + '\n' + errors
    f = open('../data/reports/bill_to_date_from_session_errors.txt','w+')
    f.write(feedback)
    f.close()
